...
public class Crosshair : MonoBehaviour
{
    ...
    public float noiseMag, noiseSpeed;
    float noisePos;
    ...
    void Update()
    {
        transform.position = Input.mousePosition + new Vector3(Mathf.PerlinNoise(noisePos, noisePos + 1000), Mathf.PerlinNoise(noisePos + 1000, noisePos + 2000), 0) * noiseMag;
        noisePos += noiseSpeed * Time.deltaTime;

        ray = textCam.ScreenPointToRay(Input.mousePosition);
        hover = Physics.Raycast(ray, out rayHit, 100.0f, textMask);
